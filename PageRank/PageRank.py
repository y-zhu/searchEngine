...
#读取graph.txt文件得到各个网页间的链接关系
def readGraph():
    fopen = open("graph.txt","r")
    for line in fopen:
        line = line.strip('\n')
...
#读取node.map.utf8得到各网页节点对应的网页名称    
def readPageName():
    fopen = open("pageName.txt","r")
    for line in fopen:
        line = line.strip('\n')
        data = re.split("-->", line) 
        if not nodeName.has_key(data[1]):
            nodeName[data[1]] = data[0]
...
def pageRank():
    ...
    fout = open("PageRankResult.txt","w")
    sum = 0
    for i in range(0 , len(ans)):
        sum += PR[ans[i][0]]
        fout.write(str(ans[i][0])+ " " + str(PR[ans[i][0]]) + " " + nodeName[ans[i][0]] +"\n")
